        ...
        try:
            with st.spinner("Procesando imagen..."):
                ...
                with st.spinner("Resolviendo juego con Deep Learning..."):
                    ...
                    # 1. Aplastamos la matriz 9x9 detectada a una lista plana de 81 números
                    lista_81 = tablero_detectado.flatten()
                    
                    # 2. Le damos forma (1, 81) según pide info_modelo.md
                    entrada_modelo = np.array(lista_81).reshape(1, 81).astype("float32")
                    
                    # 3. Cargamos el nuevo modelo de resolución
                    modelo_resolver = tf.keras.models.load_model("cnn_1m.keras")
                    
                    # 4. Hacemos la predicción y nos quedamos con el número con más probabilidad por celda
                    prediccion = modelo_resolver.predict(entrada_modelo, verbose=0)
                    lista_resuelta = np.argmax(prediccion, axis=-1)
                    
                    # 5. Volvemos a darle forma de matriz 9x9 
